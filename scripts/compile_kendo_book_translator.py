#!/usr/bin/env python3
"""Compile a self-contained Gemini Kendo Book Translator agent package.

This script uses the agent compiler to create a distributable package
for the kendo book (PDF) translation task. The resulting package includes:
...
Usage:
    python scripts/compile_kendo_book_translator.py [--output-dir DIR]
"""
...
def compile_kendo_book_translator(output_dir: Path) -> Path:
    """Compile and package the Gemini kendo book translator agent."""
    ...
    req = UserRequirements(
        ...
        package_name="gemini_kendo_book_translator",
    ...
    # 3. Override config for kendo book task
    config_kwargs = {
        "headless": True,
        "timeout": 600,
    ...
    pkg_dir = packager.package(compiled, components, req, output_dir, "gemini_kendo_book_translator")
    ...
    if dict_src.exists():
        ...
        # Fallback: look for plain name
        dict_alt = PROJECT_ROOT / "storage" / "test_srt_files" / "Trilingual Kendo Dictionary.md"
        if dict_alt.exists():
            shutil.copy2(dict_alt, pkg_dir / "kendo_dict.md")

    # 7. Copy the translation prompt template
    prompt_src = PROJECT_ROOT / "storage" / "test_srt_files" / "Trilingual Kendo Translation Prompt (1).md"
    if prompt_src.exists():
        shutil.copy2(prompt_src, pkg_dir / "translation_prompt.md")
    else:
        prompt_alt = PROJECT_ROOT / "storage" / "test_srt_files" / "Trilingual Kendo Translation Prompt.md"
        if prompt_alt.exists():
            shutil.copy2(prompt_alt, pkg_dir / "translation_prompt.md")
    ...
    print(f"\n   To use:")
    print(f"   1. Copy PDF files into: {pkg_dir / 'books/'}")
...
def _write_run_script(pkg_dir: Path) -> None:
    """Write the production agent.py for kendo book translation."""
    script = textwrap.dedent('''\
        #!/usr/bin/env python3
        """Kendo Book Translator — Self-contained Gemini translation agent.

        Translates Japanese kendo book PDFs to trilingual (JA/EN/ZH) format
        ...
        from universal_agents.core.book_prompts import (
            build_book_system_prompt,
            build_book_continue_prompt,
        ...
        async def translate_book(
            ...
            """Translate a complete book PDF, page by page."""
            ...
            try:
                async with agent:
                    ...
                    for page_num in range(start_page, end_page + 1):
                        ...
                        # ---- Build prompts ----
                        # On the first turn of ANY conversation (including after resume),
                        # send the full system prompt with dictionary + instructions.
                        if is_first_turn_in_convo:
                            ...
                            if last_completed == 0:
                                ...
                                system_prompt = build_book_new_conversation_prompt(
                                    str(DICT_FILE),
                                    str(PROMPT_FILE) if PROMPT_FILE.exists() else None,
                                    book_title=pdf_path.stem,
                        ...
                        continue_prompt = build_book_continue_prompt(page_num, total_pages)
        ...
        def _write_output(
            ...
            lines = [
                f"# {book_id} — Trilingual Translation",
                f"",
        ...
        async def main():
            parser = argparse.ArgumentParser(
                description="Kendo Book Translator — Gemini Trilingual Translation",
...
def _write_readme(pkg_dir: Path) -> None:
    """Write the README.md for the book translator package."""
    readme = textwrap.dedent("""\
        # Gemini Kendo Book Translator

        Self-contained compiled agent that translates Japanese kendo book PDFs into
        trilingual (JA / EN / ZH) format using **Gemini Pro** via browser automation.
        ...
        Trilingual sentence-by-sentence format:
        ...
        ```bash
        python scripts/compile_kendo_book_translator.py
        ```
    """)
...
def main():
    parser = argparse.ArgumentParser(description="Compile Kendo Book Translator agent")
    parser.add_argument(
        "--output-dir", "-o",
    ...
    args = parser.parse_args()
    compile_kendo_book_translator(Path(args.output_dir))
